 ...
 */
export async function scrapeTokopediaPrice(
  query: string,
  _existingPage?: any,
  _retries = 1,
  ...
  const shortQuery = query.replace(/^(ASUS|MSI|Gigabyte|ASRock|Colorful|Galax|Zotac|Sapphire|PowerColor|Palit|EVGA)\s+/i, '').trim();
  if (shortQuery !== query) {
